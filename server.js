...
db.serialize(() => {
  ...
  db.get('SELECT COUNT(*) AS c FROM teams', (err, row) => {
    ...
    if (row.c === 0) {
      ...
      const stmt = db.prepare('INSERT INTO teams(code,name,color1,color2) VALUES (?,?,?,?)');
  ...
  db.get('SELECT COUNT(*) AS c FROM fixtures', (err, row) => {
    if (err) throw err;
    if (row.c === 0) {
      const parse = (round, list) => list.map(pair => {
        const [home, away] = pair.split('v');
        return { round, home: home.trim(), away: away.trim() };
      });
      const rounds = [
      ...
      const istmt = db.prepare('INSERT INTO fixtures(round,home_code,away_code) VALUES (?,?,?)');
      rounds.forEach(r => r.forEach(m => istmt.run(m.round, m.home.replace(/\s/g,''), m.away.replace(/\s/g,''))));
      istmt.finalize();
    }
  });
});
...
function getAllFixtures() {
  return new Promise((resolve, reject) => {
    db.all(
      'SELECT f.*, th.name AS home_name, ta.name AS away_name FROM fixtures f JOIN teams th ON f.home_code = th.code JOIN teams ta ON f.away_code = ta.code ORDER BY round, id',
      (err, rows) => { if (err) reject(err); else resolve(rows); }
    );
  });
}

function getFixturesForRound(round) {
  return new Promise((resolve, reject) => {
    db.all(
      'SELECT f.*, th.name AS home_name, ta.name AS away_name, th.color1 AS h1, th.color2 AS h2, ta.color1 AS a1, ta.color2 AS a2 FROM fixtures f JOIN teams th ON f.home_code = th.code JOIN teams ta ON f.away_code = ta.code WHERE round = ? ORDER BY id',
      [round],
      (err, rows) => { if (err) reject(err); else resolve(rows); }
    );
  });
...
function computeLadderForRound(round, teams, fixtures) {
  ...
  fixtures
    .filter(m => m.round <= round && m.home_goals !== null && m.away_goals !== null)
    .forEach(m => {
      const home = table[m.home_code]; const away = table[m.away_code];
...
app.post('/admin/save', async (req,res)=>{
  ...
  for (const [k,v] of Object.entries(body)) {
    ...
    const col = m[2] === 'home' ? 'home_goals' : 'away_goals';
    const goals = v === '' ? null : Math.max(0, parseInt(v,10));
    updates.push({ id, col, goals });
  }
  db.serialize(()=>{
    const stmtHome = db.prepare('UPDATE fixtures SET home_goals=? WHERE id=?');
    const stmtAway = db.prepare('UPDATE fixtures SET away_goals=? WHERE id=?');
    updates.forEach(u=>{
      if (u.col==='home_goals') stmtHome.run(u.goals,u.id);
      else stmtAway.run(u.goals,u.id);
    });
    stmtHome.finalize(); stmtAway.finalize();
